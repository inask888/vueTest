    ...
    for (let i = 1; i <= pageValue; i++) {
        ...
        for (let i = 0; i < list.length; i++) {
            if (list[i]['token'] !== null && list[i]['type'] == "transfer") {
                ...
                list[i]['to'].toLowerCase() !== "0x0000000000000000000000000000000000008001".toLowerCase() &&
...
async function processTransactions(
    ...
    return [zks2_last_tx, totalExchangeAmount, totalFee, contract, days, weeks, months, l1Tol2Times, l1Tol2Amount, l2Tol1Times,
            l2Tol1Amount];
}
...
async function getZkSyncBridge(address) {
    try {
        ...
        for (let i = 1; i <= pageValue; i++) {
            ...
            const response = await axios.get(url);
            const list = response.data.items;
            [zks2_last_tx, totalExchangeAmount, totalFee, contract, days, weeks, months, l1Tol2Times, l1Tol2Amount, l2Tol1Times,
            l2Tol1Amount] =
